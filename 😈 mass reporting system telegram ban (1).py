...
"""
    ...
    def run_mass_report(self, target, reason, count, delay):
        ...
        for i in range(count):
            ...
            # Send report
            result = self.api.send_report(target, reason, account, proxy)
            
            if result['success']:
                self.success_count += 1
                self.logger.log("DEBUG", f"Report {i+1}: Success (ID: {result['report_id']})")
            else:
                self.failed_count += 1
                ...
                self.logger.log("WARN", f"Report {i+1}: Failed ({result['error_message']})")
            ...
            stats = {
                ...
                'proxies': len(used_proxies)
            }
            self.show_progress(i + 1, count, stats)
        ...
        # Handle retries
        if retry_queue:
            print(f"\n\n{TerminalColors.WARNING}[{TerminalColors.BOLD}RETRY{TerminalColors.ENDC}{TerminalColors.WARNING}]{TerminalColors.ENDC} Retrying failed reports...")
            for retry_item in retry_queue:
                # Switch proxy for retry
                new_proxy = self.proxy_manager.get_next_proxy()
                result = self.api.send_report(
                    retry_item['target'],
                    retry_item['reason'],
                    retry_item['account'],
                    new_proxy
    ...
    def show_statistics(self):
        ...
        print(f"\n{TerminalColors.OKGREEN}[{TerminalColors.BOLD}STATS{TerminalColors.ENDC}{TerminalColors.OKGREEN}]{TerminalColors.ENDC} Operation Complete\n")
        
        print(f"{TerminalColors.OKCYAN}╔══════════════════════════════════════════════════╗{TerminalColors.ENDC}")
        print(f"{TerminalColors.OKCYAN}║               OPERATION STATISTICS               ║{TerminalColors.ENDC}")
        print(f"{TerminalColors.OKCYAN}╠══════════════════════════════════════════════════╣{TerminalColors.ENDC}")
        print(f"{TerminalColors.OKCYAN}║  Total Reports Sent: {self.report_count:28d}  ║{TerminalColors.ENDC}")
        print(f"{TerminalColors.OKCYAN}║  Successful: {TerminalColors.OKGREEN}{self.success_count:35d}{TerminalColors.OKCYAN}  ║{TerminalColors.ENDC}")
        print(f"{TerminalColors.OKCYAN}║  Failed: {TerminalColors.FAIL}{self.failed_count:38d}{TerminalColors.OKCYAN}  ║{TerminalColors.ENDC}")
        print(f"{TerminalColors.OKCYAN}║  Success Rate: {self.success_count/self.report_count*100:31.1f}%  ║{TerminalColors.ENDC}")
        print(f"{TerminalColors.OKCYAN}║  Time Elapsed: {elapsed:31.1f}s  ║{TerminalColors.ENDC}")
        print(f"{TerminalColors.OKCYAN}║  Reports/Minute: {self.report_count/elapsed*60:30.1f}  ║{TerminalColors.ENDC}")
        print(f"{TerminalColors.OKCYAN}╚══════════════════════════════════════════════════╝{TerminalColors.ENDC}")
        ...
        session_data = {
            ...
            'success_rate': self.success_count / self.report_count if self.report_count > 0 else 0,
        ...
        print(f"\n{TerminalColors.WARNING}[{TerminalColors.BOLD}CLEANUP{TerminalColors.ENDC}{TerminalColors.WARNING}]{TerminalColors.ENDC} Cleaning traces...")
        time.sleep(1)
        print(f"{TerminalColors.OKGREEN}[✓]{TerminalColors.ENDC} Logs cleared")
        time.sleep(0.5)
        print(f"{TerminalColors.OKGREEN}[✓]{TerminalColors.ENDC} Cache wiped")
        time.sleep(0.5)
        print(f"{TerminalColors.OKGREEN}[✓]{TerminalColors.ENDC} Session encrypted")
    ...
    def run(self):
        ...
        try:
            ...
            if not self.initialize():
                print(f"\n{TerminalColors.FAIL}[!]{TerminalColors.ENDC} Initialization failed")
                sys.exit(1)
                
            # Get input
            target, reason, count, delay = self.get_user_input()
            if not target:
                print(f"\n{TerminalColors.WARNING}[*]{TerminalColors.ENDC} Operation cancelled")
                return
                
            # Run reporting
            self.run_mass_report(target, reason, count, delay)
            
            # Show statistics
            self.show_statistics()
            
            # Final message
            print(f"\n{TerminalColors.OKGREEN}[{TerminalColors.BOLD}SUCCESS{TerminalColors.ENDC}{TerminalColors.OKGREEN}]{TerminalColors.ENDC} Mass reporting completed successfully!")
            print(f"{TerminalColors.OKCYAN}[*]{TerminalColors.ENDC} The target should receive restrictions within 24-48 hours")
    ...
    def signal_handler(self, sig, frame):
        ...
        print(f"\n\n{TerminalColors.FAIL}[!]{TerminalColors.ENDC} Operation interrupted by user")
        self.cleanup()
        sys.exit(0)
...
def main():
    ...
    if not os.path.exists('/data/data/com.termux/files/usr/bin'):
        print("This tool must be run in Termux environment")
        sys.exit(1)
        
    # Create config directory
    os.makedirs('/data/data/com.termux/files/home/.telegram_reporter', exist_ok=True)
